  ...
  <div class="chart layout-version-one">
    <div class="layout-version-one-search">
        <el-form :inline="true" :model="searchForm">
            <el-form-item>
            <el-button :class="{ 'btn-active': currentIndex == 'day' }" @click="dateSearch('day', 3)">
                3天
            </el-button>
            <el-button
                :class="{ 'btn-active': currentIndex == 'week' }"
                @click="dateSearch('week', 7)"
            >
                周
            </el-button>
            <el-button
                :class="{ 'btn-active': currentIndex == 'month' }"
                @click="dateSearch('month', dayjs().daysInMonth() ?? 30)"
            >
                月
            </el-button>
            </el-form-item>
          ...
          <div class="layout-version-one-main-right-item">
            ...
            <div class="layout-version-one-main-right-item">
                ...
                <div 
                    ...
                    :key="item.key" 
                    :class="{ 'highlight-station': item.id == currentActiveStationId }"
...
const currentIndex = ref(initData.currentIndex)
...
const dateSearch = (type: string, number: number) => {
    // console.log('type', type);
    // console.log('number', number);
    currentIndex.value = type
    searchForm.value.time = getDayTime(number);
    getStationData();
}
...
const handleResetClick = () => {
    currentIndex.value = initData.currentIndex;
    searchForm.value = { ...initData.searchForm };
    getStationData();
}
...
watch([currentActiveStationId, stationData], ([
  newId, newData
]) => {
